...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark ">
    ...
    @if (session()->has('exito5'))
        <div class="container">
            <div class="alert alert-success">{{session('exito5')}}</div>
        </div>
    @endif
    ...
    <form method="POST" action="{{url('realizarEliminacion')}}">
	{{csrf_field()}}
	<div class="container h-100">
    <br>
	<div class="container">
    <h1>Centrales que tienen asociaciones para eliminar</h1>
   <table class="table table-dark table-hover">
    <thead>
      <tr>
        <th>Nombre descriptivo</th>
        <th>Localizacion geografica</th>
        <th>Tranferencia de KW</th>
        <th>Seleccion</th>
      </tr>
    </thead>
    <tbody>
    ...
    @foreach ($cents as $cent)
            @if(($cent->transmissionKW)>0)
                ...
                <td>
                    <input type="radio" name="asociador" value={{$cent->geographicLocation}}>
                </td>
            @endif
        </tr>
    @endforeach
